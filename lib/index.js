...
module.exports = exports = function(options) {
    ...
    function transform(files, encoding, callback) {
        if (files.length === 0) {
            return;
        }
        ...
        try {
            var convertedFiles = container.convert();
            convertedFiles.forEach(function(converted) {
                var filename = converted.filename;
                if (convertedFiles.length === 1 && !filename) {
    ...
    return new Combine(gutil.buffer(), through.obj(transform));
};
